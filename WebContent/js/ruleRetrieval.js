...
let TIME_FINISH;
 ...
 function getTheProjects() {
	...
	$.ajax({
		...
		url: 'Project',
		data: {
			'action': 'GET_ALL_PROJECTS_WITH_RULE',
			...
			if (response.responseJSON) {
				...
				for(var i = 0; i < response.responseJSON.length; i++) {
					...
					PROJECT_OBJECT.PROJECTS[i].lowScore = response.responseJSON[i][9];
					PROJECT_OBJECT.PROJECTS[i].mediumScore = response.responseJSON[i][10];
					PROJECT_OBJECT.PROJECTS[i].highScore = response.responseJSON[i][11];
					PROJECT_OBJECT.PROJECTS[i].scoreLastUpdated = response.responseJSON[i][12];
				}
